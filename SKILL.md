...
- **You can only see what is large on the page.** A render is looked at at
  ...
  recognisable is a part, drawn in its own crop at the scale where its
  smallest feature is at least ~40px; the only other thing an object can be
  is welded into a mass with no marks of its own. There is no middle tier.
...
| 10 | **Correct** | `stage="correct"` marks aimed by the judges, N rounds fixed at stage 0 | `describe.sh drawing.png` matches the acceptance list clause by clause; `--registration`, `--parts`, `--ranking` |
...
| S2 | **Masses** | `stage="blockin"` straights for the welded shapes, then their flats `stage="fill"` — the middle tone over the whole, then lights, then darks, honouring sharp/lost. No object yet | `--masses` against the subject at thumbnail size; **no object has an ink contour** (`--ladder` shows ink 0) |
...
- **The tracer's points are measurements, not marks.** Choose which to keep.
...
- **Straights are `smooth=False`.** `closed=True` must not repeat its first
...
- **Draw a weight ladder first**, one *vertical* stroke per `nib=`, spaced
  ...
  part's crop, at the part's scale, and `place` brings the widths back. A
...
other drops is not a verdict either way. Run it on part crops too: a face can
...
| `--parts parts.json` | a part that is absent, or drifted out of its box; each sentence printed over its crop |
